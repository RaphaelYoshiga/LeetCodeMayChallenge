...
func TestCanDoCourses(t *testing.T) {
	tables := []struct {
		a int
		b [][]int
		exp bool
		...
		{ 3, [][]int { {1,0}, {2,0}, {0,2}}, false},
		{ 4, [][]int {{0,1},{3,1},{1,3},{3,2}}, false},
	...
	for _, table := range tables {
		result := canFinish(table.a, table.b);

        if result != table.exp {
			t.Errorf("Wrong result for can finish courses %d %d\n  actual %t \n should be %t", table.a, table.b, result, table.exp);
         }
	}
}